...
const GhostCursor = ({
  ...
  const fragmentShader = `
    ...
    vec4 blob(vec2 p, vec2 mousePos, float intensity, float activity) {
      float radius = 0.8 + 0.2 * (1.0 / iScale); // Torch radius
      float dist = length(p - mousePos);
      
      // Clean, smooth radial falloff for the flashlight effect
      float distFactor = 1.0 - smoothstep(0.0, radius * activity, dist);
      float alpha = pow(distFactor, 1.5); // Parabolic falloff for realistic light
