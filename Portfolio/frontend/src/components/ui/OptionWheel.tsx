...
const OptionWheel: React.FC<OptionWheelProps> = ({
  ...
  const posRef = useRef(controlledSelectedIndex ?? defaultSelected);
  const targetRef = useRef(controlledSelectedIndex ?? defaultSelected);
  const rafRef = useRef<number | null>(null);
  const lastRef = useRef(0);
  const cfgRef = useRef<any>({});
  const onChangeRef = useRef(onChange);
  const selectedRef = useRef(controlledSelectedIndex ?? defaultSelected);
  ...
  const [selectedIndex, setSelectedIndex] = useState(controlledSelectedIndex ?? defaultSelected);
  ...
  const applyTarget = useCallback(
    (value: number, snap: boolean, isExternal: boolean = false) => {
      const cfg = cfgRef.current;
      let v = value;
  ...
  useEffect(() => {
    if (controlledSelectedIndex !== undefined && controlledSelectedIndex !== selectedRef.current) {
      applyTarget(controlledSelectedIndex, true, true);
    }
  }, [controlledSelectedIndex, applyTarget]);
