...
export default function Terminal() {
  ...
  return (
    <div className="w-full max-w-md bg-[#111111] border-4 md:border-8 border-black dark:border-white shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] md:shadow-[16px_16px_0px_0px_rgba(0,0,0,1)] dark:shadow-[12px_12px_0px_0px_rgba(255,255,255,1)] md:dark:shadow-[16px_16px_0px_0px_rgba(255,255,255,1)] transition-all duration-300 flex flex-col z-10 relative overflow-hidden" onClick={() => inputRef.current?.focus()}>
      {/* Terminal Header */}
      <div className="bg-white dark:bg-black border-b-4 md:border-b-8 border-black dark:border-white px-3 py-2 md:px-4 md:py-3 flex items-center justify-between sticky top-0 z-20">
        <div className="flex gap-2">
          <div className="w-4 h-4 md:w-5 md:h-5 bg-[#FF0055] border-2 md:border-4 border-black dark:border-white rounded-full"></div>
          <div className="w-4 h-4 md:w-5 md:h-5 bg-[#FFEB3B] border-2 md:border-4 border-black dark:border-white rounded-full"></div>
          <div className="w-4 h-4 md:w-5 md:h-5 bg-[#00FF66] border-2 md:border-4 border-black dark:border-white rounded-full"></div>
        </div>
        <span className="text-black dark:text-white font-black uppercase text-[10px] md:text-xs tracking-widest">guest@himangshu:~</span>
      </div>
      
      {/* Terminal Body */}
      <div 
        ref={containerRef}
        className="p-4 md:p-6 font-mono text-xs md:text-sm leading-relaxed flex flex-col gap-2 md:gap-3 h-[300px] md:h-[400px] overflow-y-auto overflow-x-hidden scrollbar-hide"
      >
        {history.map((item) => (
